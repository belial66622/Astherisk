...
public class DialogbySignal : InspectOnly
{
    public DialogueGraph _graph, afterdialogue,_dialogue;
    Coroutine _parser;
    ...
    {
        BaseNode baseNode = _dialogue.current;
        ...
        if (dataParts[0] == "DialogueNode")
        {
            ...
            currentDialogue++;
            int totalNodes = _dialogue.nodes.Count - 1;
    ...
    public void NextNode(string fieldName)
    {
        ...
        foreach (NodePort p in _dialogue.current.Ports) 
        {
            if(p.fieldName == fieldName)
            {
                _dialogue.current = p.Connection.node as BaseNode;
                break;
            }
        }
    ...
    {
        foreach (BaseNode b in _dialogue.nodes)
        {
